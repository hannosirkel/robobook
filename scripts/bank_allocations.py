...
def _economic_tuple(record: dict[str, Any]) -> tuple[str, str, Decimal]:
    ...
    return event_date, currency, _decimal(record.get("gross_amount"), label="Bank record gross_amount")
...
def _bank_records(paths: list[Path]) -> dict[str, dict[str, Any]]:
...
def _validate_against_normalized(allocations: list[dict[str, Any]], records: dict[str, dict[str, Any]]) -> None:
    for allocation in allocations:
        statement_id = _text(allocation.get("statement_id"))
...
def load_bank_allocations(path: Path, *, normalized_year_paths: list[Path]) -> dict[str, Any]:
    ...
    _validate_against_normalized(allocations, _bank_records(normalized_year_paths))
    return payload
...
def rebind_bank_allocations(payload: dict[str, Any], normalized_year_paths: list[Path]) -> dict[str, Any]:
    ...
    old_records = _bank_records(old_paths)
    new_records = _bank_records(normalized_year_paths)
